...
@section('content')
    ...
    <div class="print-header">
        ...
        <p>เลขที่ #{{ str_pad($borrowing->id, 4, '0', STR_PAD_LEFT) }} | พิมพ์เมื่อ
    ...
    <div class="print-content">
        ...
        <div class="print-section">
            ...
            <div class="print-row">
                ...
                <div class="print-value">
                    ...
                    @endphp
                    @if($isOverdue)
                        <span style="color: #ef4444; font-weight: bold;"> (เกินกำหนด
                            {{ now()->diffInDays(\Carbon\Carbon::parse($borrowing->due_date)) }} วัน)</span>
                    @endif
                </div>
            </div>
            <div class="print-row">
        ...
        <div class="print-section">
            ...
            <div class="print-row">
                ...
                <div class="print-value"><strong>{{ $borrowing->employee->firstname }}
                        {{ $borrowing->employee->lastname }}</strong></div>
            </div>
                    ...
                    @foreach($borrowing->items as $item)
                        <tr>
                            ...
                            <td style="text-align: center;">
                                <strong>{{ $item->quantity_requested - $item->quantity_returned }}</strong></td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
                    <tr>
                        ...
                        <th style="text-align: center;">
                            {{ $borrowing->items->sum(fn($i) => $i->quantity_requested - $i->quantity_returned) }}</th>
                    </tr>
                </tfoot>
            </table>
        </div>
        ...
        <div class="col-md-6">
            <div class="erp-card mb-3">
                ...
                <div class="erp-card-body">
                    <div
                        style="background: var(--bg-surface); border: 1px solid var(--border); border-radius: 10px; overflow: hidden;">
                        <div style="display: flex; flex-direction: column;">
                            <div class="info-row" style="display: flex; border-bottom: 1px solid var(--border);">
                                <div class="info-label"
                                    style="width: 150px; padding: 12px 16px; background: var(--bg-raised); font-size: 12px; color: var(--text-muted); font-weight: 500; flex-shrink: 0;">
                                    <i class="fas fa-file-invoice me-1" style="color: var(--accent);"></i>เลขที่ใบยืม
                                </div>
                                <div class="info-value"
                                    style="flex: 1; padding: 12px 16px; font-size: 14px; font-weight: 600; color: var(--text-primary);">
                            ...
                            <div class="info-row" style="display: flex; border-bottom: 1px solid var(--border);">
                                ...
                                <div class="info-value"
                                    style="flex: 1; padding: 12px 16px; font-size: 13px; color: var(--text-secondary);">
                            ...
                            <div class="info-row" style="display: flex; border-bottom: 1px solid var(--border);">
                                ...
                                <div class="info-value"
                                    style="flex: 1; padding: 12px 16px; font-size: 13px; color: var(--text-secondary); display: flex; align-items: center; gap: 8px;">
                                    {{ \Carbon\Carbon::parse($borrowing->due_date)->format('d/m/Y') }}
                                    @php
                                        $isOverdue = in_array($borrowing->status, ['approved', 'returned_partial']) && now()->gt(\Carbon\Carbon::parse($borrowing->due_date));
                                    @endphp
                                    @if($isOverdue)
                                        <span class="erp-badge"
                                            style="background: rgba(239,68,68,0.12); color: #f87171;">เกินกำหนด</span>
                            ...
                            <div class="info-row" style="display: flex; border-bottom: 1px solid var(--border);">
                                ...
                                <div class="info-value"
                                    style="flex: 1; padding: 12px 16px; display: flex; align-items: center;">
                                    @php
                    ...
                    <div
                        ...
                        <div style="display: flex; flex-direction: column;">
                            ...
                            <div class="info-row" style="display: flex; border-bottom: 1px solid var(--border);">
                                ...
                                <div class="info-value"
                                    style="flex: 1; padding: 12px 16px; font-size: 14px; color: var(--text-primary); font-weight: 500;">
                                    ...
                                    <tr>
                                        <td style="text-align: center;">
                                            @if($item->item && $item->item->image_url)
                                                ...
                                                <div
                                                    style="display: none; width: 40px; height: 40px; border-radius: 6px; background: var(--input-bg); align-items: center; justify-content: center; color: var(--text-muted); margin: 0 auto;">
                                                    <i class="fas fa-image" style="font-size: 14px;"></i>
                                                </div>
                                            @else
                                                <div
                                                    style="width: 40px; height: 40px; border-radius: 6px; background: var(--input-bg); display: flex; align-items: center; justify-content: center; color: var(--text-muted); margin: 0 auto;">
                                                    <i class="fas fa-image" style="font-size: 14px;"></i>
                                                </div>
                                            @endif
                                        </td>
                                        ...
                                        <td style="text-align: center;">
                                            @if($item->quantity_returned >= $item->quantity_requested)
                                                <span class="erp-badge"
                                                    style="background: rgba(52,211,153,0.12); color: #34d399;">คืนแล้ว</span>
                                            @else
                                                <span class="erp-badge"
                                                    style="background: rgba(251,191,36,0.12); color: #fbbf24;">กำลังยืม</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <th style="text-align: center; color: var(--text-primary);">
                                        {{ $borrowing->items->sum('quantity_requested') }}</th>
                                    <th style="text-align: center; color: var(--text-primary);">
                                        {{ $borrowing->items->sum('quantity_returned') }}</th>
                                    <th style="text-align: center; color: var(--text-primary);">
                                        {{ $borrowing->items->sum(fn($i) => $i->quantity_requested - $i->quantity_returned) }}
                                    </th>
                                    <th></th>
                                </tr>
                            </tfoot>
                ...
                <div class="erp-card-body">
                    <div class="d-flex justify-content-end gap-2">
                        @if(in_array($borrowing->status, ['approved', 'returned_partial']))
                            <a href="{{ route('employee.borrowing.return', $borrowing->id) }}" class="erp-btn-primary">
